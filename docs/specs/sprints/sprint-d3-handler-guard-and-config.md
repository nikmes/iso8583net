...
| D3-3 | Add `EnableDialectValidation` (default `true`) to `PipelineOptions` and `appsettings.json`; thread it into `ISOMessageFieldsPackager` / `DialectValidator` | `src/ISO8583Server/Pipeline/PipelineOptions.cs`, `tools/ISO8583Service/appsettings.json` | Not started |
...
- With `EnableDialectValidation=false`, the previous permissive behavior is restored.
