...
| D1-4 | Replace hardcoded `"1800"` with `"1804"` in `PipelineHost.BuildRequest` | `src/ISO8583Server/Pipeline/PipelineHost.cs` | Not started |
...
| D1-8 | Unit tests: `Set(0, "1800")` throws; `Pack` of `1804` + mandatory F7/F11/F24 succeeds; `Pack` missing mandatory field throws; integration test that server-initiated echo now emits `1804` (not `1800`) | `tests/ISO8583Net.Tests/`, `tests/ISO8583Service.Tests/` | Not started |
