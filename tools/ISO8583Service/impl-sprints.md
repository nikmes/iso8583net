...
| S3-5 | Register `DefaultHandler` in DI as catch-all, register `Iso8583HostedService` unchanged | `Program.cs` | ⬜ |
...
| S4-2 | Implement `SendSignOnOnConnect` — after TLS handshake, push initial SignOn to writer channel before reader starts | `PipelineHost.cs` | ⬜ |
...
| ID | Task | File(s) | Status |
|----|------|---------|--------|
| S5-1 | Add `PipelineBenchmarks` to `ISO8583Net.Benchmarks` — measure throughput (msg/sec) with varying concurrency | `benchmarks/ISO8583Net.Benchmarks/` | ⬜ |
| S5-2 | Benchmark: single connection, 1 parser task vs 2 vs 4 parser tasks | — | ⬜ |
...
|--------|------|-------|--------|
| 0 | Groundwork | 9 | ⬜ |
| 1 | Reader & Writer Stages | 8 | ⬜ |
| 2 | Parser Stage | 7 | ⬜ |
| 3 | Dispatcher & Handlers | 9 | ⬜ |
| 4 | SignOn, Echo & Periodic | 9 | ⬜ |
| 5 | Tuning & Benchmarks | 7 | ⬜ |
| 6 | Hardening & Polish | 9 | ⬜ |
| **Total** | | **58 tasks** | **0%** |
...
- **S0** must finish first (all sprints depend on new types)
- **S1 + S2** can run in parallel (reader/writer and parser are independent stages)
- **S3** needs S1 + S2 (needs parsed messages flowing through pipeline)
- **S4** can start after S1 (only needs writer channel), but also needs S3 for handler registration
- **S5** needs S3 (needs working end-to-end pipeline to benchmark)
